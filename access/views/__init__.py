...
from access.models import ActiveToken, LoginLog
from access.tokens import PreAuthToken
from access.serializers import LoginSerializer, UserSerializer
...
def validate_pin(pin):
    """
    Перевіряє PIN на відповідність вимогам.
    Повертає рядок з помилкою або None якщо PIN валідний.
    """
    if not pin.isdigit():
...
class CookieJWTAuthentication(JWTAuthentication):
    """
    Читає access_token з httpOnly куки (або Authorization хедера).
    Якщо кука присутня але невалідна — кидає AuthenticationFailed,
    щоб downstream код ніколи не отримав AnonymousUser з активною кукою.
    """
